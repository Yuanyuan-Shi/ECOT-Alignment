...
def refined_objective(actions, vectors, stop_mask, threshold=.5, stop_tolerance=.03, stop_loss="threshold_hinge"):
    ...
    if stop_loss == "net_squared":
        stop_penalty=(magnitude/stop_tolerance).square()
    elif stop_loss == "threshold_hinge":
        stop_penalty=torch.relu(magnitude/stop_tolerance-1).square()
    else:
        raise ValueError("Unknown stop loss: " + stop_loss)
    zero=actions.sum()*0
