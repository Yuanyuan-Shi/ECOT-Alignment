#!/usr/bin/env python3
"""Capture policy query 1 for λ=2 task 24, state 14, seed 80."""
...
OUTPUT = ROOT / "reports/lambda2_task24_state14_seed80_query1_trace"
TARGET_GENERATION_CALL = 41  # 40 queries in state 13, then state 14 query 0.
TARGET_POLICY_QUERY_INDEX = 1
EXPECTED_ACTION_TOKENS = [151723, 151869, 151763, 151796, 151785, 151673, 151895]
...
def main() -> None:
    ...
    def traced_raw_generate(self: OpenVLA, input_ids, pixel_values):
        ...
        top2 = []
        for action_position, position in enumerate(range(action_start, action_stop)):
            score_vector = generated.scores[position][0].float()
            values, indices = torch.topk(score_vector, k=2)
            record = {
                    "generated_position": position,
                    "action_token_position": action_position,
            ...
            if action_position == 4:
                record["requested_candidate_logits_float32"] = {
                    "151785_reference": float(score_vector[151785]),
                    "151746_aws_math_sdpa": float(score_vector[151746]),
                }
                record["requested_candidate_margin_reference_minus_aws"] = float(
                    score_vector[151785] - score_vector[151746]
                )
            top2.append(record)
        ...
        trace = {
            "case": {
                "task_id": 24,
                "trial": 1,
                "initial_state_index": 14,
                "episode_seed": 80,
                "policy_query_index": TARGET_POLICY_QUERY_INDEX,
            },
    ...
    frame = (
        OUTPUT
        / "evaluation/rank-0/assets/libero_90-task24-episode1-seed80-query001-before.png"
    )
    if not frame.is_file():
        raise FileNotFoundError(frame)
    shutil.copy2(frame, OUTPUT / "task24_state14_seed80_query001.png")

    rows = [json.loads(line) for line in (OUTPUT / "evaluation/rank-0/alignment_queries.jsonl").open()]
    target = next(
        row
        for row in rows
        if row["episode_id"] == "libero_90-task24-episode1-seed80"
        and row["policy_query_index"] == TARGET_POLICY_QUERY_INDEX
    )
    (OUTPUT / "query001_record.json").write_text(json.dumps(target, indent=2) + "\n")
    ...
    summary = {
        ...
        "camera_png_sha256": sha256_file(OUTPUT / "task24_state14_seed80_query001.png"),
        ...
        "query001_record_sha256": sha256_file(OUTPUT / "query001_record.json"),
