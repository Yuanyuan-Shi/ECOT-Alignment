...
Q=ROOT/'runs/refined-stop-pooled'
...
def summarize(rows):
    ...
    result['alignment_loss']=(sum(ds)+sum(ss))/(len(ds)+len(ss)) if ds or ss else 0
    return result
...
def main():
    ...
    cfg=m.MoveAlignmentFinetuneConfig(
        ...
        run_name='refined-stop-pooled-lr5e5-'+args.mode+'-seed20261102',seed=20261102,
    ...
    config={**asdict(cfg),'alignment_objective':'pooled_mean_eligible_directional_and_literal_stop_squared_hinges',
