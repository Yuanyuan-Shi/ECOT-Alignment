...
OUT=ROOT/'analysis_results/move_only_500/fixed_training_outcome_comparison.json'
...
for name in ('Original MiniVLA','Round 3 lambda=1','Mismatch only'):
    ...
    with torch.no_grad():
        for idx,batch in enumerate(loader):
            ...
            for rid, cosine, vector, success in zip(batch['record_ids'],captured['cosines'].cpu().tolist(),batch['reasoning_move_vector'].cpu().tolist(),batch['episode_success'].cpu().tolist()):
                query_scores.append({'id':rid,'cosine':cosine,'directional':any(vector),'episode_success':bool(success),'mismatch':cosine<.5})
    ...
    score_path=OUT.parent/('training_query_scores_'+name.lower().replace(' ','_').replace('=','')+'.json')
