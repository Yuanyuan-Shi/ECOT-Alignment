...
python -u vla-scripts/finetune_move_alignment.py \
  ...
  --run_name "round3-ecot-lite-libero90-hinge-lora-r16-lambda${suffix}-seed20261102" \
  --wandb_project ecot-move-alignment --wandb_mode online \
  --seed 20261102 --validation_fraction 0.2 \
  --epochs 13 --batch_size "$batch" --gradient_accumulation_steps "$accum" \
  ...
  --save_every_epochs 100 --max_optimizer_steps 500 --performance_loss_weight 0 --final_train_validation True \
