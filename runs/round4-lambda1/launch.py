...
STATE = ROOT / 'runs/round4-lambda1/status.json'
if STATE.exists():
...
processes = {}
for name in ('failure-weight3', 'balanced-sampling'):
    run_dir = f'runs/ecot-move-alignment/round4-lambda1-restart-{name}'
    evaluation_root = f'experiments/robot/libero/results/round4-move-eval-lambda1-{name}-90task-2trial-seed20261201'
    gate = f'runs/round4-lambda1/start-{name}'
    if any((ROOT / p).exists() for p in (run_dir,evaluation_root,gate)):
        raise FileExistsError(f'Refusing to overwrite {name}')
    state['runs']['round4-lambda1-'+name] = {'lambda': 1.0, 'display_name': 'Round 4 lambda=1.0 — '+name,
...
report()
for key,run in state['runs'].items():
    with open(f"runs/round4-lambda1/train-{run['name']}.log",'x') as log:
        p = subprocess.Popen(['bash','runs/round4-lambda1/train.sh',run['name'],'--start_gate',run['start_gate']],
...
while True:
    previous = json.loads((ROOT/'runs/round4/status.json').read_text())
...
for key,run in state['runs'].items():
    ...
    try:
        ...
        report()
        with open(f"runs/round4-lambda1/eval-{run['name']}.log",'x') as log:
            p=subprocess.Popen(['bash','runs/round4-lambda1/evaluate.sh',run['name']],stdout=log,stderr=subprocess.STDOUT)
        run['process_pid']=p.pid
        save()
        if p.wait():
