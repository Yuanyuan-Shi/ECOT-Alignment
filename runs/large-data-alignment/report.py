...
def main():
    ...
    if plan.get('wandb_project'):
        project_url='https://wandb.ai/'+plan['wandb_entity']+'/'+plan['wandb_project']
        parts.append('<p>W&B project: <a href="'+html.escape(project_url)+'">'+html.escape(plan['wandb_project'])+'</a></p>')
    ...
    for key in ['lambda0','lambda1','moveonly']:
        ...
        if w:parts.append(' · W&B run '+html.escape(w['id'])+' ('+html.escape(w.get('mode','offline'))+'; local logs retained)')
