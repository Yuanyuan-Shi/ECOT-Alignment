...
def sync(key):
    ...
    paths=sorted((rd/'wandb').glob('offline-run-*'))
    ok=True
    for path in paths:
        try:
            with (Q/'wandb-sync.log').open('a') as f:
                p=subprocess.run(['/home/exx/.conda/envs/openvla/bin/wandb','sync',str(path)],stdout=f,stderr=subprocess.STDOUT,timeout=120)
            ok &= p.returncode==0
