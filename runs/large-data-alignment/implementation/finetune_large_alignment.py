...
def backward_effective_batch(vla,batch,cfg):
    ...
    for start in range(0,size,32):
        ...
        with torch.autocast('cuda',dtype=torch.bfloat16):
            ...
            loss=align*aw if cfg.performance_loss_weight==0 else perf*pw if cfg.move_loss_weight==0 else perf*pw+align*aw
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--mode',choices=['lambda0','lambda1','moveonly'],default='lambda1');p.add_argument('--probe',action='store_true');args=p.parse_args()
    plan=json.loads((Q/'plan.json').read_text());seed=plan['training_seed'];m.set_seed(seed)
    assert plan['batch_size']==32 and plan['gradient_accumulation_steps']==1
    cfg=m.MoveAlignmentFinetuneConfig(checkpoint=Path(plan['base_checkpoint']),batch_size=plan['batch_size'],gradient_accumulation_steps=1,learning_rate=plan['learning_rate'],move_loss_weight=0. if args.mode=='lambda0' else 1.,performance_loss_weight=0. if args.mode=='moveonly' else 1.,seed=seed)
    rd=ROOT/f'runs/ecot-move-alignment/large-data-{args.mode}-b32-s2000';rd.mkdir(parents=True,exist_ok=True)
    ...
    if args.probe:
        ...
        records=[json.loads(l) for l in (Q/'records.jsonl').read_text().splitlines()];train=[x for x in records if x['split']=='training'];val=[x for x in records if x['split']=='validation']
    ...
    # Offline logging never blocks computation when either network interface drops.
    run=m.wandb.init(project='ecot-move-alignment',name=rd.name,mode='offline',config={**plan,'mode':args.mode},dir=str(rd))
    write(rd/'wandb.json',dict(id=run.id,directory=str(Path(run.dir).parent),project='ecot-move-alignment'))
    def log(payload):
        def flatten(value,prefix=''):
            ...
            return out
        run.log(flatten(payload),step=payload['optimizer_step'])
        with (rd/'metrics.jsonl').open('a') as f:f.write(json.dumps(payload)+'\n')
        write(rd/'progress.json',payload)
    step=0;epoch=0;cursor=0
    resume=rd/'resume.pt'
    if resume.exists():
        x=torch.load(resume,map_location='cpu',weights_only=False);assert x['mode']==args.mode and x['base_checkpoint']==plan['base_checkpoint'] and x.get('loss_version')==plan['loss_version']
    ...
    def snapshot():
        payload=dict(loss_version=plan['loss_version'],mode=args.mode,base_checkpoint=plan['base_checkpoint'],step=step,epoch=epoch,cursor=cursor,adapter={k:v.detach().cpu().clone() for k,v in get_peft_model_state_dict(vla.llm_backbone.llm).items()},optimizer=optimizer.state_dict(),python_rng=random.getstate(),numpy_rng=np.random.get_state(),torch_rng=torch.get_rng_state(),cuda_rng=torch.cuda.get_rng_state_all())
    ...
    while step<plan['max_steps']:
        ...
        if step%250==0:
            ...
            log(dict(optimizer_step=step,validation=metrics));snapshot()
    with torch.random.fork_rng(devices=[0]):
