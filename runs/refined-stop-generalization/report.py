...
ROOT=Path(__file__).resolve().parents[2];Q=ROOT/'runs/refined-stop-generalization';OUT=ROOT/'analysis_results/refined_stop_generalization'
...
def rollout(root,plan):
    ...
    for line in (root/'alignment_queries.jsonl').read_text().splitlines():
        ...
        delta=a[:,:3].sum(0);c=float(v@delta/(np.linalg.norm(v)*np.linalg.norm(delta)+1e-8))
        ...
        rows.append({'episode_success':outcomes[r['episode_id']],'eligible':directional or stop,'stop':stop,'directional':directional,
                     'mismatch':bool(c<.5) if directional else bool(np.linalg.norm(net)>.03+1e-8) if stop else False})
    result={}
...
def main():
    ...
    base=ROOT/'runs/ecot-move-alignment/refined-stop-lr5e5-lambda1-seed20261102/pretrained-training-summary.json'
    for key in NAMES:
    ...
    text=['<h1>Refined directional/stop loss: lower LR and validation early stopping</h1>',
          '<p>Fresh original MiniVLA initialization for both tuned models. λ=1 retains supervised loss; mismatch-only excludes it. Alignment loss is the sum of separate directional and literal-stop group means. Direction: squared hinge at cosine .5; stop: squared excess net-command magnitude above .03 m. Stop commands are clipped and scaled by .05 m per input unit; numerical boundary tolerance 1e-8 m. Other zero-vector claims are excluded from alignment.</p>',
